...
class Stone(Animation):
    ...
    def __init__(self):
        ...
        self.init_animation() #Animation클래스로부터 init_animation() 호출.
               
    def update(self): 
        # 돌은 투석기가 발사한 이후부터 목표 또는 
        # 지면에 충돌할 때까지 날기만 하므로
        # 상태에 따른 스프라이트 정지 등은 필요 없음.
        #update()가 호출되면 시간의 흐름에 따라 애니메이션 프레임변경하고,self.image갱신.
    ...
    def move(self, time, space, decrement_stones):
        ...
        #Stone이 화면 밖을 벗어나면 READY로 바꾸고 매개변수인 decrement_stones()호출.
