...
def decrement_stones(): #stone_count변수를 1감소.
    global stone_count
    stone_count -= 1

class Background(Sprite): #배경 이미지를 이동시키는 클래스.
    def __init__(self):        
...
if __name__ == "__main__": 
    ...
    game_state = GAME_INIT #게임 상태를 GAME_INIT로 초기화
    #Background 스프라이트와 그룹을 생성.
    ...
    while run:
        ...
        if game_state == GAME_PLAY:
            # 누르고 있는 키 확인하기.
            #pygame.key.get_pressed()는 혀재 사용자가 계속해서 누르고 있는 키를 가져옴.
            keys = pygame.key.get_pressed()  
            if keys[pygame.K_LEFT]:
                catapult.backward()
            elif keys[pygame.K_RIGHT]:
                catapult.forward()
            elif keys[pygame.K_UP]: #발사각을 증가.
                if direction < MAX_DIRECTION:
                    direction += 1
            elif keys[pygame.K_DOWN]: #발사각을 감소.
                if direction > MIN_DIRECTION:
                    direction -= 1    
            elif keys[pygame.K_SPACE]: #누르고 있는 동안에는 발사 파워를 증가.
                if power > MAX_POWER:
                    power = MIN_POWER
                else:
                    power += 0.2
        ...
        # 2) 게임 상태 업데이트
        if stone.state == STONE_FLY: #FLY이면 t를 증가시키고, move()를 호출
            t += 0.5
            stone.move(t, 
        ...
        if game_state == GAME_INIT: 
            ...
            if not alien.alive(): #만약 돌에 맞아 죽었다면 explosion재생.
